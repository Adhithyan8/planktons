...
for dataset in datasets:
    ...
    def data_transform(img, label):
        ...
        img = A.PadIfNeeded(img.shape[1], img.shape[0], border_mode=4)(
            image=img
        )[
            "image"
        ]
    ...
    # save everything
    np.save(f"outputs/ViT_DINOv2_{dataset}_trn_old_out.npy", trn_old_out)
    np.save(f"outputs/ViT_DINOv2_{dataset}_trn_new_out.npy", trn_new_out)
    np.save(f"outputs/ViT_DINOv2_{dataset}_tst_old_out.npy", tst_old_out)
    np.save(f"outputs/ViT_DINOv2_{dataset}_tst_new_out.npy", tst_new_out)
    np.save(f"outputs/ViT_DINOv2_{dataset}_trn_old_lbl.npy", trn_old_lbl)
    np.save(f"outputs/ViT_DINOv2_{dataset}_trn_new_lbl.npy", trn_new_lbl)
    np.save(f"outputs/ViT_DINOv2_{dataset}_tst_old_lbl.npy", tst_old_lbl)
    np.save(f"outputs/ViT_DINOv2_{dataset}_tst_new_lbl.npy", tst_new_lbl)
