"""
We try prior matching with a power law distribution, this time with label loss
...
class DINO(L.LightningModule):
    def __init__(
        ...
        self.criterion = DINOLoss(
            ...
            teacher_temp=0.04,
            center_momentum=1.0,  # turn off centering by setting momentum to 1
...
def remap_labels_PLANKTON(img, label):
    (img0, img1), label = data_transform(img, label)
    # remap the label
    label = SORTED_PLANKTON_CLS.index(label)
    label = torch.tensor(label, dtype=torch.long)
    return (img0, img1), label
...
for dataset in datasets:
    ...
    if dataset == "HERB19":
        trn_old_dataset = make_dataset(
            info, split_fit="train", split_cat="old", transform=remap_labels_HERB19
        )
    elif dataset == "PLANKTON":
        trn_old_dataset = make_dataset(
            info, split_fit="train", split_cat="old", transform=remap_labels_PLANKTON
        )
    else:
        trn_old_dataset = make_dataset(
    ...
    # save the model
    torch.save(model.state_dict(), f"outputs/exp_08_{dataset}.pt")
